...
public class NewGroupe extends JFrame implements ActionListener{
    ...
    Object[][] tableContent = new Object[50][3] ;
    JTable contactTable;
    JScrollPane scroll;
    JButton saveGroup,cancel;
    DefaultTableModel model;
NewGroupe(contactList cl){
    contactl=cl;
    setSize(630,500);
    fillLeftGroupe();
    fillRightGroupe();
  ...
  @Override
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        contactl.addGroup(groupName, groupDesc);
        for(int i=0;i<contactTable.getRowCount();i++){
        if(contactTable.getValueAt(i, 2).equals(true)){
        contactl.addContactToGroupe(contactList.getContacts().get(i),contactList.getGroups().get(contactList.getGroups().size()-1));       
        }
        }
